...
import { Step1VendorDirectory } from "./Step1VendorDirectory";
import { Step2Contracts } from "./Step2Contracts";
import { Step3SpendAnalysis } from "./Step3SpendAnalysis";
import { VendorManagementPreview } from "./VendorManagementPreview";
import { VendorManagementData, INITIAL_DATA } from "./types";
...
const WIZARD_STEPS = [
    {
        ...
        description: "Manage your vendors and contacts",
        component: Step1VendorDirectory
    },
    {
        id: "step-2",
        title: "Contracts",
        description: "Track contract terms and renewals",
        component: Step2Contracts
    },
    {
        id: "step-3",
        title: "Spend Analysis",
        description: "Analyze vendor spending patterns",
        component: Step3SpendAnalysis
    }
];

interface VendorManagementWizardProps {
    initialData?: VendorManagementData;
    onSave?: (data: VendorManagementData) => Promise<void>;
    onReset?: (profileId: string) => Promise<any>;
    onComplete?: (data: VendorManagementData) => void;
    onCancel?: () => void;
}

export function VendorManagementWizard({ initialData, onSave, onReset, onComplete, onCancel }: VendorManagementWizardProps) {
    ...
    const [data, setData] = useState<VendorManagementData>(initialData || INITIAL_DATA);
    ...
    const handleUpdate = useCallback((updates: Partial<VendorManagementData>) => {
    ...
    const handleNext = async () => {
        if (currentStep < WIZARD_STEPS.length - 1) {
            ...
            try {
                ...
                toast.success("Vendor Management Profile Completed", {
                    description: "Your vendor data has been successfully saved."
                });
    ...
    return (
        !isMounted ? <WizardSkeleton /> :
            <ModuleErrorBoundary name="Vendor Management Module">
                <TooltipProvider>
                    <div className="w-full max-w-5xl mx-auto p-4 md:p-6 lg:p-8">
                        {/* Header */}
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                            <div>
                                <h1 className="text-3xl font-bold font-display tracking-tight">Vendor Management</h1>
                                <p className="text-muted-foreground mt-1">Track vendors, contracts, and spending.</p>
                            </div>
                        ...
                        {isPreviewMode ? (
                            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                                <VendorManagementPreview data={data} mode="preview" />
                            </div>
                        ) : (
                        ...
                        <AlertDialog open={showResetConfirm} onOpenChange={setShowResetConfirm}>
                            <AlertDialogContent>
                                <AlertDialogHeader>
                                    <AlertDialogTitle>Delete All Vendor Data?</AlertDialogTitle>
                                    <AlertDialogDescription>
                                        This will permanently delete all your vendor profiles, contracts, and spend records. This action cannot be undone.
                                    </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
